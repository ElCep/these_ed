...
to install-trapping ;;observer context
  ...
  if rational-traps = 3 [ ;;disposition organiser à l'échelle de la PARCELLE
    ...
    foreach list-parcelles [
      [ x ] ->
      let myparcelle patches with [numParcelle = x AND numParcelle != 0]
      let surface-lute count myparcelle
      let surface-trap pi * (radius-trap ^ 2)
      if any? myparcelle with [not any? other traps with [parcelleID = x] in-radius radius-trap] [
        while [surface-lute > surface-trap AND any? myparcelle with [not any? other traps with [parcelleID = x] in-radius radius-trap]][
          if any? myparcelle with [not any? other traps with [parcelleID = x] in-radius radius-trap][
            let goodplaceFotraping one-of myparcelle with [not any? other traps with [parcelleID = x] in-radius radius-trap]
            create-traps 1 [
              set color red
              set parcelleID x
              move-to goodplaceFotraping
    ...
    if rational-traps = 4 [ ;;disposition organiser à l'échelle de la zone mais avec des passagé clandestin
      ...
      [ x ] ->
       ask patches with [numParcelle = x][
         set clandestin? TRUE
         if any? traps-here[
          ask  traps-here [
           die
