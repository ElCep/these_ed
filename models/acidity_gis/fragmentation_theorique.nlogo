...
to go
  ask n-of nb_plots patches with[zoneUrba = FALSE AND pcolor = black][
   set pcolor red 
  ]
  calcul-frac
end
...
to calcul-frac
  ...
  find-clusters
  set Frag (max [plabel] of patches + 1);CYRIL: tu pourrais mettre let counter 1 dans show-clusters pour ne pas avoir à rajouter + 1 ici, non? -> OUI c'est vrai :-D
end

to find-clusters
  loop [
    let seed one-of patches with [(cluster = nobody) and pcolor = red] ;CYRIL: il manquait les deux dernières conditions ici!!
    if seed = nobody
    [ show-clusters
      stop ]
    ask seed
...
to show-clusters
  let counter 0
  loop
  [ let p one-of patches with [(plabel = "" ) and pcolor = red] ;CYRIL: il manquait les deux dernières conditions ici!!
    if p = nobody
      [ stop ]
    ask p
...
;    (pcolor = [pcolor] of myself)]
  ask neighbors4 with [(cluster = nobody) and pcolor = red]
...
185
184
potentiel patches viti
count patches with [zoneUrba = FALSE]
2
1
11
...
<experiments>
  <experiment name="experiment" repetitions="50" runMetricsEveryStep="true">
    ...
    <timeLimit steps="1"/>
    <metric>Frag</metric>
    <steppedValueSet variable="nb_plots" first="1" step="1" last="5244"/>
  </experiment>
</experiments>
@#$#@#$#@
@#$#@#$#@
