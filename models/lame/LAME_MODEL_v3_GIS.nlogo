...
globals[
  ...
  ;;GIS DATA VARIABLE
  slope  ;; needed for load slope from GIS data
  parcelles
  acces ;;charge le raster de distance
  ...
  list_slope_failed_plots ;;liste des pentes des parcelles viticole qui sont abandonée 
...
to setup-env
 ...
 ask patches [
  set dist_v gis:raster-sample acces patch pxcor pycor
 ]
 ...
 create-farmers NbFarmer [
   ...
   set mySlope [slope_here] of patch-here
...
to setupGIS
  ...
  set parcelles gis:load-dataset "data_carto/ParcellesVitiSQLITE/small_parcelles_AOCWGS84.shp"
...
;  gis:set-world-envelope (gis:envelope-of parcelles)
;  gis:apply-coverage parcelles "COMMUNE" cat
;  
  
  ;;pour charger les pentes 
  set slope gis:load-dataset "data_carto/DTMBanyulsEPSG2154/small_slope.asc"
  set acces gis:load-dataset "data_carto/small_interpolation_pgrouting_IDW_cost.asc"
  ;;pour simplement dessiner le raster
;  gis:paint slope white

;  ;;pour attribuer les valeur du raster aux patches 
  let maxSlope max[slope_here] of patches
  ask patches[
    set slope_here gis:raster-sample slope patch pxcor pycor
    ifelse slope_here != 256 [
      set pcolor scale-color red slope_here 0 90
    ][
      set slope_here 100
      set pcolor black
    ]
  ]
  ;C'est ici qu'on fait discuter les patches et le shp
;  ask patches gis:intersecting parcelles [
;    set pcolor categorie
;    set commune? TRUE
;    ]
...
to calculCost ;farmer context, myplots context
    ...
    set annualCost cout_ha + (coef_Slope * slope_here) + (coef_dist * dist)
end
...
to updateEmployer ;vitis context
  if any? myplots [
    set meanSlope (mean [slope_here] of myPlots) ;la pente est en % le +0.01 est pour interdire les divisions par 0
    if meanSlope = 0 [
     set  meanSlope 10
    ]
    let wk (1 / ((100 * (tan meanSlope)) / 100))
...
to stat_1
  if any? farmers with [color != white][
    set prop_farmer_in_hill ((count farmers with[meanSlope > 6 AND color != white]) / (count farmers - (count Farmers with [not any? myplots])))
  ]
  if any? patches with [owner != -1 AND owner != NOBODY][
    set med_slope median[slope_here] of patches with [owner != -1 AND owner != NOBODY]
  ...
  if ticks = 50 [
    set list_slope_failed_plots [slope_here] of patches with [owner = -1]
    set list_slope_viti [slope_here] of patches with [owner != -1 AND owner != NOBODY]
    set list_dist_v [dist_v] of patches with [owner != -1 AND owner != NOBODY]
  ]
   set list_slope_echange_plots [slope_here] of patches with [nb_echanges > 0]
   set list_dist_echangePlots [dist_v] of patches with [nb_echanges > 0]
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot vitipatches"
...
@#$#@#$#@
<experiments>
  <experiment name="experiment-find-alpha-beta-GIS" repetitions="20" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="51"/>
    ...
    <enumeratedValueSet variable="GIS?-i">
      <value value="true"/>
    </enumeratedValueSet>
  ...
  <experiment name="experiment-long-time-GIS" repetitions="20" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="100"/>
    <metric>gini-index-reserve</metric>
